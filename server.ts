...
// API: Validate URL (CORS bypass and server-side resolution verification)
app.post("/api/validate-url", async (req: express.Request, res: express.Response): Promise<any> => {
  const { url } = req.body;
  ...
  try {
    ...
    // We will attempt to run a HEAD or GET fetch to check if the URL resolves.
    const controller = new AbortController();
    const timeoutId = setTimeout(() => controller.abort(), 5000);

    try {
      // Use standard fetch. Try HEAD request first for efficiency.
      let response = await fetch(parsedUrl.toString(), {
        method: "HEAD",
        signal: controller.signal,
        headers: {
          "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) UTMBuilderValidator/1.0",
        }
      });

      // If HEAD fails with 405 (Method Not Allowed) or similar, fallback to GET
      if (!response.ok && [404, 405, 501, 403].includes(response.status)) {
      ...
      if (fetchErr.name === "AbortError") {
        return res.json({
          valid: false,
          error: "Timeout: The URL took too long to respond.",
        });
      }
      return res.json({
        valid: false,
        error: fetchErr.message || "Network error: Failed to reach the server.",
      });
    }
  } catch (err: any) {
    return res.json({
      valid: false,
      error: "Malformed URL: Please check the format (include http:// or https://).",
    });
  }
});
